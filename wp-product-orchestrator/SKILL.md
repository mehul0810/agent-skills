...
- Default implementation, CI triage, dependency resolution, investigation, and proof to bounded workers. Route by artifact: plugin -> `wp-plugin-expert`; theme/FSE -> `wp-theme-expert`; site -> `wp-site-expert`; copy -> `content-writer`; brand/product video -> `product-video-producer`; upstream contribution -> `wp-contributor`.
