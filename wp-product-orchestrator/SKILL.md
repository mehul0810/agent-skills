...
- Own backlog strategy, design gaps, dependency/stale-PR hygiene, WordPress.org/support visibility, and evidence. Design-change PRs need screenshot proof or proof gap.
...
- Use delta-first product reporting. Include current objective, active PR/issues, blockers, owner-gated items, Codex-handled items, validation evidence, next action, and stop condition; compress quiet status to one sentence.
...
- Prefer localhost `wp-proof` for readiness, screenshots, package validation, and workflow proof. Use Studio for parity or local data.
