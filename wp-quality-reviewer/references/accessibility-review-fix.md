...
- WordPress accessibility coding standard: https://developer.wordpress.org/coding-standards/wordpress-coding-standards/accessibility/
...
- Target standard and review boundary. A scoped review is not a conformance audit; do not infer site-wide conformance from sampled components.
...
- Authentication (`3.3.8`): test login, OTP/MFA, and recovery, not just the password field. Support password managers and paste, including full-code paste into segmented OTP inputs. A cognitive test requires a non-cognitive alternative, an assistance mechanism, object recognition, or identification of the user's own non-text content. These exceptions do not waive other criteria; do not impose a blanket CAPTCHA ban or excuse blocked assistance as security policy.
...
- Pointer: use no down-event activation, abort/undo on up-event completion, or up-event reversal unless essential (`2.5.2`). Provide single-pointer, non-path alternatives to multipoint/path gestures (`2.5.1`) and dragging (`2.5.7`); device-motion actions need UI alternatives and disable controls subject to the criterion's exceptions (`2.5.4`).
...
- Separate tests: `200%` text resize (`1.4.4`) and reflow (`1.4.10`) at `320` CSS-pixel width for horizontal writing, `256` CSS-pixel height for vertical writing. A `1280px` viewport at `400%` zoom exercises horizontal reflow; `200%` zoom alone does not. Exceptions apply to genuinely two-dimensional content, not the entire surrounding page.
...
- Formal audits require `accessibilityAudit` in the quality-report schema and validator: all 55 A/AA criteria exactly once (`4.1.1` is obsolete; AAA is separate), with status, concrete rationale, and reusable check IDs. The `checks` ledger includes `fullPages`, `completeProcesses`, `accessibilitySupport`, `nonInterference`, `keyboard`, and `assistiveTechnology`; add only necessary checks and reuse evidence pointers rather than pasting logs. Record expected/observed results, environment, and artifacts. Missing tests are `blocked`, not `not_applicable`.
