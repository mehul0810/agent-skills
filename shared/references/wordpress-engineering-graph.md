...
Release/elevated graphs add root source identity and align critical proof to it; critical regression proof needs run ID/environment. The validator rejects orphaned nodes, unresolved critical states, conflicting exclusive owners, stale dependencies, unproved acceptance, fabricated/mismatched evidence, and outcomes without upstream proof. Store compact evidence pointers; a syntactically valid SHA/URL does not prove identity, freshness, trust, or privacy.
