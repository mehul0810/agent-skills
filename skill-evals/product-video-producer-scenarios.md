...
| Reference-led direction | "Use these enterprise videos to make our launch film feel equally premium." | Inspects timestamped source evidence, extracts principles without copying identity/sequence, and validates a representative sequence before scaling; delegated internal selection does not require redundant owner approval. |
...
| Non-destructive editing | "Turn these mixed phone captures and screen recordings into a clean software demo." | Inventories source identity/rights/cadence/color/audio, conforms variable-frame-rate media, preserves originals and proxy lineage, versions rough/fine cuts, requires assigned-reviewer picture lock before final post, preserving explicit owner checkpoints, and proves review/master derivation. |
...
- **Motion cost boundary:** Poster-only correction does not trigger a 4K rerender. Use current owner model/effort policy and supported runtime controls. Do not silently substitute another family or claim a prompt changed runtime settings.

## Consistent Quality Cases

- **Delegated versus reserved direction:** Delegated brand-consistent direction proceeds through internal representative-sequence review. An explicit request to show alternatives and wait still stops before selection.
- **Model-neutral acceptance:** Lower effort or a cheaper permitted model cannot waive failed playback, source fidelity, rights or output identity. Repair or permitted escalation is required; unresolved quality remains draft. Never promise identical creativity or universal model equivalence.
- **Crop and zoom budget:** A 3840x2160 raster cropped to half width and enlarged into a full-width UHD shot is deficient even if the container probes as 4K. Embedded rasters inside SVG are not exempt. Validate actual encoded detail separately from arithmetic coverage.
- **Creative range:** Enterprise art direction need not become the same minimal text/card template. Choose purposeful imagery and distinctive composition against the brief without inventing product evidence.
- **Cheap preview, expensive proof:** A clean low-resolution preview does not prove native 4K edge/compression quality; render representative full-resolution ranges. A poster-only edit does not rerender unrelated scenes.
- **Cross-model measurement:** Compare identical frozen inputs/checks across allowed settings; separate evaluator detection from generation quality. Record failures, retries and unavailable metrics honestly.
