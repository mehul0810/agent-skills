...
| Setup-blocked cooldown | "Run the 10-minute product heartbeat again; exact saved project is still missing and nothing changed." | `delegation-protocol.md` | Skips full rehydration and worker retries, returns concise `DONT_NOTIFY/setup-blocked`, names the missing exact repo-root project, and states the next owner action. |
...
- Did setup-blocked product heartbeats return `DONT_NOTIFY/setup-blocked` instead of repeated full rehydration or worker retries when no new source-of-truth signal exists?
