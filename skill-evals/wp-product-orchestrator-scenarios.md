...
| Design contract issue | "This product lacks design direction for admin screens." | `repo-product-docs-contract.md` | Creates or adapts `DESIGN.md` only through a duplicate-screened issue when a real durable design gap exists; keeps it concise, uses the WordPress Design System as the default baseline, and layers product brand colors/identity on top. |
| Repo-doc audit | "This repo has no AGENTS.md and the UI workflow keeps drifting." | `repo-product-docs-contract.md` | Audits active-product repo docs, creates focused issues for weak/missing/stale `AGENTS.md`, `DESIGN.md`, `TESTING.md`, or `RELEASE.md`, and keeps cross-product orchestration rules in the skill pack rather than the product repo. |
...
| Owner unavailable unblock | "The owner is offline and this product heartbeat is blocked on a reversible local proof-environment/config choice." | `product-autonomy-permissions.md` | Does not stop at wait-state language; verifies repo/product reality, chooses the safest reversible unblock path, proceeds with evidence, and reports only the residual owner-only follow-up. |
...
| Adjacent maintainability finding | "While fixing the issue, the worker finds weak non-obvious comments, repeated validation surprises, and missing tests outside scope." | `adjacent-finding-protocol.md` | Keeps the PR scoped, duplicate-screens, creates or links a focused issue unless the finding is fixed inside the PR, includes symptom/impact/source/files/acceptance/validation/risk/docs-update fields, and updates product docs only when the lesson is repo-specific and reusable. |
...
- Did repo-doc audits cover weak/missing/stale `AGENTS.md`, `DESIGN.md`, `TESTING.md`, and `RELEASE.md`, creating focused issues instead of relying on chat memory?
- Did `DESIGN.md` treat the WordPress Design System as the default baseline and product brand colors as the product-specific layer?
...
- Did it actively unblock non-destructive, local, reversible, non-hard-gated blockers instead of stopping just because the owner was unavailable?
...
- Did unexpected behavior, code smell, weak non-obvious comments, maintainability debt, missing tests, repeated validation/tooling surprises, or workflow friction become focused duplicate-screened issues unless fixed inside the current PR scope?
- Did fixed-in-scope adjacent findings appear in PR summary/validation, and did reusable repo-specific lessons update product docs instead of chat memory?
