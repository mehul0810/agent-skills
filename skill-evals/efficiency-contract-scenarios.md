# Efficiency Contract Scenarios

| Request | Required observable decision |
|---|---|
| Extend an approved design system with a new page | Reuse established direction; resolve only material composition uncertainty using a wireframe or rendered prototype. Do not require generated imagery unless requested or necessary. |
| Build five pages from an exact target | Preserve the target and validate a representative WordPress section with real assets, editor controls, and responsive behavior before scaling. |
| Review a new PR head using an existing task packet | Reuse valid decisions; verify live state, invalidate affected evidence, and append current-head dispositions without reconstructing the whole plan. |
| Report token savings from passing word budgets | State that budgets alone do not prove savings; compare available usage, duration, handoffs, retries, and reopened defects on equivalent work. |

Rendered defect discrimination uses [frontend-benchmark/CONTRACT.md](frontend-benchmark/CONTRACT.md) and captured variants. Keep visual craft and usability grades separate. Native WordPress ownership remains blocked until actual import/editor/frontend proof exists.
